  ...
  <div class="row d-flex justify-content-between">
    ...
    <div class="col-md-4 col-sm-6">
      <b-form @reset="onReset" v-if="show">
        ...
        >
          <option
            v-for="centro in centrosPublicados"
            :key="centro.id"
            v-bind:value="centro"
          >
            {{ centro.nombre }}
    ...
    <div class="col-md-7 col-sm-6">
      <ve-ring :data="chartData" :loading="loading">
        <div v-if="dataEmpty" class="data-empty">
          Seleccione un centro y una fecha para ver el gráfico
        </div>
      </ve-ring>
    </div>
  </div>
    ...
    async onInput() {
      if (this.form.centro && this.form.fecha) {
        ...
        this.chartData = {
          columns: ['tipo', 'cant'],
          rows: [
            { tipo: 'Disponible', cant: resultado.data.cant },
